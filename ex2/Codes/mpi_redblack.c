...
int main(int argc, char ** argv) {
	...
	for (t=0;t<T;t++) {
		...
		// Before Red is executed we have to send black cells
		// As a result we send receive black cells we all the neighbors we have (if there are any neighbors)
		if(north!=MPI_PROC_NULL) {
		        MPI_Sendrecv(&(u_previous[1][1]),1,row,north,rank,&(u_previous[0][1]),1,row,north,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		if(west!=MPI_PROC_NULL) {
		        MPI_Sendrecv(&(u_previous[1][1]),1,column,west,rank,&(u_previous[1][0]),1,column,west,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		if(south!=MPI_PROC_NULL){
		        MPI_Sendrecv(&(u_previous[local[0]][1]),1,row,south,rank,&(u_previous[local[0]+1][1]),1,row,south,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		if(east!=MPI_PROC_NULL){
		        MPI_Sendrecv(&(u_previous[1][local[1]]),1,column,east,rank,&(u_previous[1][local[1]+1]),1,column,east,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		// After message passing has been completed we compute red cells
		...
		// Now we have to send receive reds to all our neighbours

		if(north!=MPI_PROC_NULL) {
		        MPI_Sendrecv(&(u_current[1][1]),1,row,north,rank,&(u_current[0][1]),1,row,north,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		if(west!=MPI_PROC_NULL) {
		        MPI_Sendrecv(&(u_current[1][1]),1,column,west,rank,&(u_current[1][0]),1,column,west,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		if(south!=MPI_PROC_NULL){
		        MPI_Sendrecv(&(u_current[local[0]][1]),1,row,south,rank,&(u_current[local[0]+1][1]),1,row,south,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		if(east!=MPI_PROC_NULL){
		        MPI_Sendrecv(&(u_current[1][local[1]]),1,column,east,rank,&(u_current[1][local[1]+1]),1,column,east,MPI_ANY_TAG,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
		}

		// After message passing has been completted we compute black cells
	...
	if (rank==0) {
		printf("RebBlackSOR X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf ConvergeTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,total_time,conv_time,U[global[0]/2][global[1]/2]);
